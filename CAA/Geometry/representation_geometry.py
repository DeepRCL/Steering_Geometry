...
def _plot_embedding_2d(out_path: Path, title: str, coords: np.ndarray) -> None:
...
def _higher_order_groups_for_value(value: str) -> set[str]:
    if value in BOUNDARY_GROUPS:
        return set(BOUNDARY_GROUPS[value])
...
def _build_theoretical_similarity_matrix(relations_path: Path) -> np.ndarray:
...
def _unit_vectors(vectors: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    unit_vectors = {}
    for value in SCHWARTZ_CIRCUMPLEX_ORDER:
        vec = vectors[value].detach().cpu().float()
        norm = vec.norm()
        unit_vectors[value] = vec / norm if norm > 0 else vec
    return unit_vectors


def mean_center_vectors(vectors: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    mean_vec = torch.stack([vectors[value].detach().cpu().float() for value in SCHWARTZ_CIRCUMPLEX_ORDER]).mean(dim=0)
    return {
        value: vectors[value].detach().cpu().float() - mean_vec
        for value in SCHWARTZ_CIRCUMPLEX_ORDER
    }
...
def analyze_representation_geometry(
    ...
    for i, v1 in enumerate(SCHWARTZ_CIRCUMPLEX_ORDER):
        for j, v2 in enumerate(SCHWARTZ_CIRCUMPLEX_ORDER):
            empirical_sim[i, j] = F.cosine_similarity(unit_vectors[v1], unit_vectors[v2], dim=0).item()

    theoretical_sim = _build_theoretical_similarity_matrix(relations_path)
    ...
    with (output_dir / "spearman_report.json").open("w") as f:
        json.dump({
    ...
    _plot_embedding_2d(output_dir / "umap_2d.png", f"{title_prefix}UMAP 2D Projection", X_umap)

    X_pca = PCA(n_components=2, random_state=seed).fit_transform(X)
    _plot_embedding_2d(output_dir / "pca_2d.png", f"{title_prefix}PCA 2D Projection", X_pca)
    ...
    _plot_embedding_2d(output_dir / "tsne_2d.png", f"{title_prefix}t-SNE 2D Projection", X_tsne)

    dist_matrix = np.maximum(0.0, 1.0 - empirical_sim)
    ...
    geometry_metrics = {
        ...
        "within_minus_across_cosine": within_group_mean - across_group_mean,
    ...
    plt.title(f"{title_prefix}2D MDS Aligned to Theoretical Circumplex", fontsize=PLOT_TITLE_FONTSIZE)
    plt.axis("equal")
    ...
    jitter = np.random.default_rng(seed).normal(0.0, 0.03, size=len(theo_flat))
    ...
    plt.title(f"{title_prefix}Empirical Similarity vs Theory")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / "theory_vs_empirical_scatter.png", dpi=300)
    plt.close()
    ...
    plt.title(f"{title_prefix}Empirical Minus Theoretical Similarity")
    plt.tight_layout()
    plt.savefig(output_dir / "empirical_minus_theoretical_heatmap.png", dpi=300)
    plt.close()

    if metadata is not None:
        with (output_dir / "representation_metadata.json").open("w") as f:
