"""
Extract per-value CAA persona vectors in the Llama-Scope SAE sparse latent space.

For each contrastive pair (pos/neg) and each Schwartz value:
  1. Run Llama forward pass.
  ...
  3. Encode through the SAE using pre_encode (dense, pre-TopK):
       pre = encoder(act)               # (32768,) — dense, continuous
...
After all pairs for a value (three sequential steps):
  ...
  Step 4 — difference vector:
       persona_vec[value] = v_pos - v_neg          shape: (32768,)
...
# Helpers
# ──────────────────────────────────────────────────────────────────────────────
def _vec_path(config: LlamaScopePipelineConfig, val: str) -> str:
    return os.path.join(config.sparse_vectors_dir, f"{safe_name(val)}.pt")


def _all_cached(config: LlamaScopePipelineConfig) -> bool:
    return all(
...
def _load_cached(config: LlamaScopePipelineConfig) -> Dict[str, torch.Tensor]:
    return {v: torch.load(_vec_path(config, v), map_location="cpu")
            for v in SCHWARTZ_CIRCUMPLEX_ORDER}


def _act_path(config: LlamaScopePipelineConfig, val: str) -> str:
...
def extract_sparse_vectors(
    ...
    """
    Compute per-value sparse CAA persona vectors in the Llama-Scope feature space.

    If sae is None, loads the fine-tuned checkpoint when present; otherwise
    downloads and uses the pre-trained Llama-Scope SAE directly.
