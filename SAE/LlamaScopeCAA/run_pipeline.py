"""
LlamaScopeCAA pipeline — CLI entry point.

Uses the Llama-Scope pre-trained SAE collection for Llama-3.1-8B residual
streams to extract sparse CAA persona vectors.

Four modules, run in order or independently:

  finetune   Adapt the pre-trained Llama-Scope SAE to value-specific residual
             activations.  Requires GPU + Llama-3.1-8B.
  ...
  extract    Compute sparse CAA persona vectors (one per Schwartz value) using
             ...
             Requires GPU + Llama.  Writes sparse_vectors_caa_base/ and
             steering_activations/.

  evaluate   Steer the model via the sparse SAE space and measure A/B logit
             accuracy.  Requires GPU + Llama.  Writes evaluation/.
...
──────────────────────────────────────────────────────────────────────────────
  With fine-tuning (default when --modules includes "finetune"):
    python -m SAE.LlamaScopeCAA.run_pipeline --layer 20 --modules all

  Without fine-tuning (use pre-trained Llama-Scope SAE directly):
    python -m SAE.LlamaScopeCAA.run_pipeline --layer 20 --skip_finetune
    # equivalently:
    python -m SAE.LlamaScopeCAA.run_pipeline --layer 20 --modules extract,evaluate,geometry
...
──────────────────────────────────────────────────────────────────────────────
  ...
  python -m SAE.LlamaScopeCAA.run_pipeline --layer 20 --modules finetune,extract,evaluate
  python -m SAE.LlamaScopeCAA.run_pipeline --layer 20 --modules geometry

  # Custom output directory (e.g. when comparing multiple layers):
  python -m SAE.LlamaScopeCAA.run_pipeline \\
    --layer 24 --skip_finetune \\
    --output_dir SAE/LlamaScopeCAA/outputs_layer24
"""
import argparse
import os
...
# Helpers
# ──────────────────────────────────────────────────────────────────────────────
def _load_vectors(config: LlamaScopePipelineConfig):
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "LlamaScopeCAA: value persona vectors in Llama-Scope SAE sparse latent space"
        ),
    ...
    p.add_argument(
        "--sae_repo",
        default="OpenMOSS-Team/Llama3_1-8B-Base-LXR-8x",
        help="HuggingFace Hub repo ID for the Llama-Scope SAE collection",
    )
    p.add_argument("--sae_expansion", type=int, default=8, help="Llama-Scope SAE expansion factor")
    p.add_argument("--sae_site", default="R", help="Llama-Scope SAE site code; R = residual stream")
    p.add_argument(
        "--layer",
        type=int,
        default=16,
    ...
    p.add_argument(
        ...
        help=(
            ...
            "The Llama-Scope SAE was trained with k=50, but any value can be used at\n"
    ...
    p.add_argument("--d_sae", type=int, default=32768, help="SAE feature dim (default: 32768)")
    ...
    p.add_argument(
        "--output_dir",
        default="SAE/LlamaScopeCAA/outputs",
        help="Root output directory (default: SAE/LlamaScopeCAA/outputs)",
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "  finetune  — fine-tune Llama-Scope SAE on value-specific activations\n"
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the fine-tuning step and use the pre-trained Llama-Scope SAE directly.\n"
...
def main() -> None:
    ...
    if "finetune" in modules:
        print("\n" + "=" * 66)
        print("  MODULE: finetune — adapt Llama-Scope SAE to value activations")
        print("=" * 66)
    ...
    if "extract" in modules:
        print("\n" + "=" * 66)
        print("  MODULE: extract — Llama-Scope sparse CAA persona vectors")
        print("=" * 66)
    ...
    if "evaluate" in modules:
        print("\n" + "=" * 66)
        print("  MODULE: evaluate — Llama-Scope SAE steering accuracy")
        print("=" * 66)
        if vectors is None:
