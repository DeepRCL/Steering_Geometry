"""
Geometry analysis for LlamaScopeCAA.
...
only the vector definition differs from the old LlamaScopeCAA behavior.
...
# LlamaScopePipelineConfig is duck-type compatible: it exposes
...
def _activation_path(config: LlamaScopePipelineConfig, value: str) -> str:
    return os.path.join(config.steering_activations_dir, f"{safe_name(value)}.pt")


def _geometry_vector_path(config: LlamaScopePipelineConfig, value: str) -> str:
    return os.path.join(config.geometry_vectors_dir, f"{safe_name(value)}.pt")


def _resolve_geometry_alpha(config: LlamaScopePipelineConfig) -> float:
    ...
    with torch.no_grad():
        for start in range(0, activations.shape[0], batch_size):
            ...
            if config.use_pre_topk_personas:
                ...
                recon = sae.decode(sae.sparsify(z + alpha * persona_vec))
...
def _compute_displacement_vectors(
    ...
    if missing:
        raise FileNotFoundError(
            "LlamaScopeCAA displacement geometry requires saved training activations. "
...
def run_geometry(
    ...
    """
    Run geometry analysis for LlamaScopeCAA.
