"""
Fine-tune the Llama-Scope pre-trained SAE on value-specific residual activations.

WHY FINE-TUNE?
──────────────────────────────────────────────────────────────────────────────
The Llama-Scope SAE was trained on general text.  Fine-tuning on the
...
PROCEDURE
──────────────────────────────────────────────────────────────────────────────
Step A — Activation collection (one Llama forward pass per prompt):
  For every row in the combined dataset, run BOTH the positive and negative
  prompts through Llama.  Hook `model.model.layers[config.layer]` and collect
...
Step B — SAE fine-tuning (CPU or GPU, Llama not needed):
  ...
  The checkpoint is saved in the Llama-Scope format {W_enc, W_dec, b_enc, b_dec}.
"""
...
from .config import LlamaScopePipelineConfig, SCHWARTZ_CIRCUMPLEX_ORDER
...
def _collect_activations(
    ...
    """
    Run all training prompts through Llama, capture every token's residual-stream
...
# Public entry point
# ──────────────────────────────────────────────────────────────────────────────
def finetune_sae(config: LlamaScopePipelineConfig) -> TopKSparseAutoencoder:
    """
    Fine-tune the Llama-Scope SAE on value-specific residual activations.
    ...
    if os.path.exists(config.finetuned_sae_path):
        ...
        from .topk_sae_model import load_llamascope_sae
        return load_llamascope_sae(config.finetuned_sae_path, k=config.k, device="cpu")
    ...
    # ── Load pre-trained Llama-Scope SAE ───────────────────────────────────────
    sae_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\nLoading pre-trained Llama-Scope SAE (layer {config.layer}) …")
    ...
    # ── Save in Llama-Scope format ─────────────────────────────────────────────
    os.makedirs(os.path.dirname(config.finetuned_sae_path), exist_ok=True)
    save_llamascope_sae(sae, config.finetuned_sae_path)
