"""
Extract per-value CAA persona vectors in the SAE sparse latent space.

For each contrastive pair (pos/neg) and each Schwartz value:
  1. Run Qwen 3.5 9B forward pass.
  2. Hook model.model.layers[mlp_layer].mlp — capture the LAST TOKEN MLP
     output (shape: (4096,)).  This is the same 4096-d space the SAE was
     trained on.
  3. Encode through the fine-tuned SAE:
       z = relu( W_enc @ (act - b_pre) )   shape: (16384,)
  4. Accumulate z for pos and neg prompts separately.

After all pairs for a value:
  persona_vec[value] = mean(z_pos_list) - mean(z_neg_list)   shape: (16384,)

This is a standard CAA difference vector, but computed in the SAE's sparse
feature space rather than the dense residual/MLP activation space.
...
# Helpers
# ──────────────────────────────────────────────────────────────────────────────
def _vec_path(config: SparseCAAPipelineConfig, val: str) -> str:
    return os.path.join(config.sparse_vectors_dir, f"{safe_name(val)}.pt")


def _all_cached(config: SparseCAAPipelineConfig) -> bool:
    return all(os.path.exists(_vec_path(config, v)) for v in SCHWARTZ_CIRCUMPLEX_ORDER)


def _load_cached(config: SparseCAAPipelineConfig) -> Dict[str, torch.Tensor]:
...
def extract_sparse_vectors(
    ...
    """
    Compute per-value sparse CAA persona vectors.

    If sae is None, loads the fine-tuned checkpoint from
    config.finetuned_sae_path.  Falls back to config.sae_checkpoint if the
    fine-tuned file does not exist (e.g., fine-tuning was skipped).
